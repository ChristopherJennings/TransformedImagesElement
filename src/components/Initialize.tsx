...
export const initElement = (element: ICustomElement, context: IContext) => {
    try {
        ...
        const endpoint = element.config.contentManagementEndpoint;

        listAssetsService(element.config.contentManagementEndpoint)
            .subscribe(
...
const parseResponse = (element: ICustomElement, context: IContext, items: AssetModels.Asset[], continuationToken: string | null, listAssetsService: IListAssetsService, contentManagementEndpoint: string) => {
    ...
    prepareElementProps(context, element.disabled, selectedImageModels, rawAssets, continuationToken, listAssetsService, contentManagementEndpoint);
}

const prepareElementProps = (context: IContext, disabled: boolean, selectedImageModels: TransformedImageModel[], rawAssets: AssetModels.Asset[], continuationToken: string | null, listAssetsService: IListAssetsService, contentManagementEndpoint: string) => {
    const selectedIds = selectedImageModels.map(i => i.id);

    const moreAssets = (continuationToken: string) => {
        return listAssetsService(contentManagementEndpoint, continuationToken);
    };
